...
class PaymentController extends Controller
{
    ...
    public function paynow(Request $request)
    {
        ...
        if (isset($request->type) && $request->type == 'deposit') {
            ...
            // Check and upgrade user designation based on deposit
            $user = auth()->user();
            ...
            // Redirect to gateway details page
            return redirect()->route('user.gateway.details', $gateway->id);
        }

        // Validate investment limits
        $plan = Plan::with('time')->findOrFail($request->plan_id);

        if ($plan->amount_type == 0) {
            if ($plan->maximum_amount && $request->amount > $plan->maximum_amount) {
                return redirect()->back()->with('error', 'Maximum Invest Limit ' . number_format($plan->maximum_amount, 2));
            }

            if ($plan->minimum_amount && $request->amount < $plan->minimum_amount) {
                return redirect()->back()->with('error', 'Minimum Invest Limit ' . number_format($plan->minimum_amount, 2));
            }
        }

        if ($plan->amount_type == 1) {
            if ($plan->amount && $request->amount != $plan->amount) {
                return redirect()->back()->with('error', 'Fixed Invest Limit ' . number_format($plan->amount, 2));
            }
        }
